...
struct point{
    ...
    const point& operator-(const point& right){
        ...
        //А ссылка начинает указывать на невыделенную память
        //есть два решения
        //1) деконструкторы объектов из кучи не вызываются.
        //2) так как сделана следующая перегрузка
    }

    const point operator*(const point& right){
        return point(point::x * right.x, point::y * right.y);
        //Нужно избавиться именно от временного объекта. Не вызывать его конструктор и в последствии деконструктор.
        //При такой записи создаваемый конструктором объект сразу присваивается безымянной переменной возврата.
        //Аналогично записи: const point 'return' = point(....);
    }

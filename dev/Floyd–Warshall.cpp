        ...
        for (NT i = 0; i < Vert_Number; ++i) {
            for (NT j = 0; j < Vert_Number; ++j) {
                ...
                if (Distance[i][j] > alternative_path_length){
                    ...
                    // Поэтому при поиске кротчайших (i->k) и (k->j) достаточно рассматривать пути на графах {i, k, 1, 2,.., k-2, k-1} и {k, j, 1, 2,.., k-2, k-1} соответственно, а не на {i, j, k, 1, 2,.., k-2, k-1}
